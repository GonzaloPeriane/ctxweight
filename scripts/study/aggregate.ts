...
function main(): void {
  ...
  const L: string[] = [];
  L.push("# ctxweight study");
  L.push("");
  L.push(
    `_Aggregated scan of **${total}** public repositories' agent context, generated ${scanDate} with [ctxweight](https://github.com/GonzaloPeriane/ctxweight). Only aggregate numbers are published — no repo content, no per-repo security findings. Reproduce with \`npm run study && npm run study:aggregate\`._`,
  );
  L.push("");
  L.push("## Coverage");
  L.push("");
